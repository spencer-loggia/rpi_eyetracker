...
four synchronized `1280x720` camera images side by side. The shipped profile
currently retains that native aggregate resolution.
...
Set `exposure_us` and `analogue_gain` in `video/example_config.json` to the
required fixed shutter and gain. Exposure must remain shorter than one frame
period (for example, below 16,667 microseconds at 60 fps). The same settings
are used by `live_preview.py`, so its image matches the recorder. Set `crf` in
the same file to control H.264
quality; it accepts integer values from 0 through 51. Lower values retain more
detail and produce larger files. The code default is 24 when the field is
omitted; the shipped test profile explicitly selects 20.
...
```bash
ffmpeg -fflags +genpts -r 60 -i test.h264 -c:v copy test.mp4
```
...
- `preset=ultrafast`: prioritizes the least CPU-intensive x264 preset. The
  configured `crf` value controls constant-quality rate control.
- `maxrate=64000000;bufsize=128000000`: limits CRF excursions. Even if the
  ...
  and other usage. Typical constant-quality output should be smaller.
...
The current 60 fps `5120x720` RAW8 stream would be about 6.37 TB uncompressed,
and even the camera's lowest native
...
The shipped profile retains the native `1280x720` image from every camera and
must be load-tested with the actual experiment. For an even lighter
`640x400` per-camera stream, use sensor mode `2560:400:8` and output size
`2560x400`.
...
```bash
ffmpeg -r 60 -i session_001.h264 -c:v libx265 -preset slow -crf 24 session_001_archive.mp4
```

If `framerate` is changed from the shipped 60 fps profile, use that value
for ffmpeg's input `-r` as well.

...
def _run_local_preview(config_path: Path) -> int:
    from .service import EyeTrackingService
    ...
    with EyeTrackingService(config, layout) as service:
        service.start_tracking()
        print("Live diagnostic preview; press Q or Esc in the window to stop", flush=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "preview":
            return _run_local_preview(args.config)
        if args.command == "run":
            return _run_service(
                args.config,
                args.start_tracking,
